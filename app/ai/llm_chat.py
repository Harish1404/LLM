...
def get_api_key_for_model(model_name: str) -> str:
    """
    Resolves the correct API key from settings based on the model provider.
    """
    ...
    try:
        ...
        if collected_chunks_list:
            try:
                ...
                metrics.log_metrics(model_name=model_name, response=complete_response_object)
...
class ChatService:
    ...
    async def get_streaming_response(self):
        ...
        for current_model in models_to_attempt:
            ...
            api_key = get_api_key_for_model(current_model)
            
            max_retries = 3
            for attempt in range(1, max_retries + 1):
                try:
                    # Prepare call arguments dynamically
                    call_kwargs = {
                        "model": current_model,
                        "messages": [
                    ...
                    api_response_stream = await acompletion(**call_kwargs)
                    ...
                    return stream_generator(api_response_stream, self.model)
